"""Woosh-DFlow FastAPI 推理服务。

服务启动时通过 :class:`api.compute_agent.MultimodelGenerateAgent` 加载模型，
请求进入内存队列后由后台 worker 串行调用生成流程，最终返回 FLAC 音频。
该文件只负责 HTTP 生命周期、排队、状态查询和音频压缩，不实现模型结构。
"""
...
class GenerateRequest(BaseModel):
    ...
    Attributes:
        version: API schema 版本。
        token: 预留鉴权字段，当前代码未验证其内容。
        args: 传给模型 agent 的生成参数。
    """
...
class ActiveRequest:
    ...
    ``result`` 保存已压缩的音频 bytes，``event`` 用于同步等待接口在后台
    生成完成后被唤醒。
    """
...
def run_generate(req: GenerateRequest):
    """调用 compute agent 执行一次生成。

    Args:
        req: FastAPI 请求体，内部的 ``args`` 会传入模型 agent。

    Returns:
        tuple[dict, float]: 生成结果 ``{"audio": Tensor, "sample_rate": int}``
        与耗时秒数。
    """
...
def compress_audio(
    ...
    """把 ``[C, T]`` waveform Tensor 压缩为内存中的音频 buffer。"""
    buf = io.BytesIO()
    torchaudio.save(
        buf,
        wav_tensor,
...
async def process_queue():
    """后台消费请求队列，串行运行模型推理并写回 ``active_requests``。"""
