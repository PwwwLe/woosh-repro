"""构建 Woosh DiT 的基础 block。

核心抽象是 ``DictTensor``：不同 modality（audio latent、description、
video_features）分别存放在不同 key 下，attention/MLP block 按 key 读取并写回。
本文件实现 rotary embedding、time embedding、单模态 attention、多模态融合
attention，以及 single-stream multimodal block。
"""
...
class ModalityAttention(nn.Module):
    """单个 modality 的 self-attention。

    输入从 ``d[x_key]`` 读取，形状为 ``[B, tokens, dim]``；可选用
    ``d[mod_key]`` 做 AdaLN-style modulation，并用 ``d[freqs_cis_key]`` 做
    rotary embedding。
    """
    ...
    def modulate(self, x, d):
        """根据 ``mod_key`` 对 layer-normalized token 加 bias/scale/gate。"""
        if self.use_modulation:
            assert self.mod_key is not None
...
class ModalityAttentionWrapper(nn.Module):
    """包装 ``ModalityAttention``，用于在不复制权重的情况下扩展行为。"""
...
class MMMAttention(nn.Module):
    """跨 modality 融合 attention。

    每个 modality 先单独计算 q/k/v，然后沿 token 维拼接，在一次
    ``scaled_dot_product_attention`` 中完成联合注意力，最后按原 modality
    长度拆回并写入各自 key。
    """
...
class MMMBlock(nn.Module):
    """把多个 ``ModalityBlock`` 融合成一个多模态 block。"""
...
class SelfAttention(nn.Module):
    """UMBlock/video encoder 使用的 DictTensor self-attention。"""
