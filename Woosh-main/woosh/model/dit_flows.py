"""Woosh-Flow 使用的 multimodal DiT 结构。

``InputProcessing`` 把 AE latent、时间步和 CLAP 文本条件转为 ``DictTensor``；
``SFXFlow`` 组装若干 ``MMMBlock`` 和 ``MultimodalitySingleStreamBlock``；
``PostProcessing`` 将 hidden token 投影回 latent velocity。
"""
...
class InputProcessing(nn.Module):
    """把 latent/时间/条件投影成 DiT blocks 使用的 ``DictTensor``。"""
    ...
    def pad_description(
        ...
        """在 no-description-mask 模式下用可学习 padding 替换无效文本 token。"""
        if self.no_description_mask:
    ...
    def embed_x(self, x):
        """把 ``[B, C, T]`` latent patchify 并投影为 ``[B, tokens, dim]``。"""
    ...
    def forward(
        ...
        ``description`` 来自 ``cond["cross_attn_cond"]``，形状从
        ``[B, text_tokens, cond_token_dim]`` 投影为 ``[B, text_tokens, dim]``。
        """
        batch_size = x.size(0)
...
class PostProcessing(nn.Module):
    """把 DiT hidden audio tokens 投影回 latent channel。"""
    ...
    def forward(self, d: DictTensor) -> DictTensor:
        """移除 memory tokens、反 patchify，并写回 ``d["x"]``。"""
        main_key = "x"
        mod_key = "t"
        x = d[main_key]
...
class SFXFlow(DiTPipeline):
    """Woosh text-to-audio 的 DiT velocity 网络。"""
