"""Woosh latent diffusion/flow pipeline。

该模块把 DiT velocity 网络、Woosh-AE 和一组 conditioner 组合成完整 LDM：
``get_cond`` 生成条件字典，``_denoise_dict_no_param`` 给 flow matching sampler
使用，``autoencoder.inverse`` 负责把生成 latent 解码成 waveform。
"""
...
class LatentDiffusionModelPipeline:
    """LDM 推理逻辑 mixin；自身不是独立 ``nn.Module``。"""
    ...
    def init_pipeline(
        ...
        """绑定 DiT、autoencoder、conditioners 和 EDM/flow 参数。"""
    ...
    def get_cond(
        ...
        """汇总所有 conditioner 的输出为 LDM 标准条件字典。"""
        cond_dict = {}
    ...
    def _batched_cond_denoise(self, x_t, t, mask=None, cond=None, cond_batched=None):
        """在一个 batch 中同时计算条件/无条件 denoise，供 CFG 使用。"""
    ...
    def _denoise_dict_no_param(self, x_t, t, cond=None, mask=None) -> DictTensor:
        """无 EDM 重参数化的 denoise，Flow matching sampler 直接使用该路径。"""
        device = x_t.device
        assert cond is not None
        if mask is None:
            if "mask" in cond:
...
class LatentDiffusionModel(nn.Module, BaseComponent, LatentDiffusionModelPipeline):
    """完整 text-to-audio Woosh-Flow 组件。"""
...
class LatentDiffusionModelFlowMapPipeline(LatentDiffusionModelPipeline):
    """带第二时间步 ``r`` 的 FlowMap pipeline mixin。"""
    ...
    def _denoise_dict_no_param(self, x_t, t, r, cond=None, mask=None) -> DictTensor:
        """FlowMap denoise：输入当前 ``t`` 与目标 ``r``，返回 ``x_hat``。"""
        assert cond is not None
        if mask is None:
            if "mask" in cond:
                mask = cond["mask"]
