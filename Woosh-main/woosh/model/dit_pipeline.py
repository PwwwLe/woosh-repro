...
``DiTPipeline`` 统一执行 preprocessing -> Transformer layers -> postprocessing。
FlowMap 版本 ``DiTFlowMapPipeline`` 只是在 forward 中额外传入第二个时间步
``r``。两个 pipeline 都通过 ``DictTensor`` 在模块间传递多模态中间表示。
"""
...
def checkpoint(function, *args, **kwargs):
    """统一设置 PyTorch checkpoint 的 ``use_reentrant=False``。"""
...
class DiTPipeline(torch.nn.Module):
    ...
    def forward(
        ...
        Args:
            x: latent 输入 ``[B, io_channels, frames]``。
            t: 时间 embedding 输入，通常为 ``[B]``。
            cond: 条件字典。
            mask: latent 有效位置 mask ``[B, frames]``。
        """
        # Creates dict
...
class DiTFlowMapPipeline(torch.nn.Module):
    """带第二时间步 ``r`` 的 distilled FlowMap DiT 管线。"""
    ...
    def forward(
        ...
        """执行 FlowMap DiT 前向，返回包含 ``x`` 的计算字典。"""
        # Creates dict
