"""Woosh-AE 使用的 Vocos-style encoder/decoder。

该文件实现从 waveform 到 STFT/mel 特征的 encoder、ConvNeXt/ResNet backbone、
多种 Fourier/ISTFT/IMDCT head，以及 ``VocosAutoEncoder`` 等组合类。
官方 Woosh-AE 配置使用 ``VocosAutoEncoder``，输入 ``[B, 1, T]``，latent 为
``[B, z_dim, frames]``，解码输出 ``[B, 1, T']``。
"""
...
class VocosEncoder(nn.Module):
    """STFT/mel 特征 + VocosBackbone + bottleneck projection 的 encoder。"""
    ...
    def forward(self, x):
        """编码 waveform ``[B, 1, T]`` 到 latent ``[B, output_channels, F]``。"""
        with torch.autocast("cuda", enabled=False):
...
class VocosDecoder(nn.Module):
    """VocosBackbone + Fourier head 的 waveform decoder。"""
    ...
    def forward(self, x):
        """解码 latent ``[B, C, F]`` 到 waveform ``[B, 1, T]``。"""
        y = self.backbone(x)
        with torch.autocast("cuda", enabled=False):
            y = self.head(y.float())
