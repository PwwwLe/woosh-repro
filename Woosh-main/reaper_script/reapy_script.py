"""REAPER 集成示例：从文本 prompt 生成音效并插入时间线。

脚本通过本地 Woosh API 请求 FLAC 音频，保存到临时目录后调用 reapy/ReaScript
把文件插入当前 REAPER project。运行前需要 API server、reapy 与 REAPER
Python 配置都可用。
"""
...
def get_tcl_lib_dirs_from_brew():
    """在 macOS/Homebrew 环境中自动查找 TCL_LIBRARY 和 TK_LIBRARY。"""
    try:
...
def generate(prompt: str) -> str:
    """调用 Woosh API 生成 FLAC 文件，并返回本地临时路径。"""
...
def insert_file_at_cursor(file_path: str):
    """把音频文件插入 REAPER 当前 project 的光标位置。"""
...
def ui_main():
    """启动一个最小 Tkinter 输入框，供 REAPER 用户输入 prompt。"""
...
def cli(prompt):
    """在无 UI 模式下生成默认 prompt 并插入 REAPER。"""
...
def entry(ui):
    """Click CLI 入口，按 ``--ui`` 决定 Tkinter 或默认 CLI 流程。"""
    if ui:
        ui_main()
    else:
