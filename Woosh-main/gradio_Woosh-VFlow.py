"""Woosh-VFlow text+video-to-audio Gradio demo。

数据流为 uploaded video -> frame extraction -> Synchformer video features ->
VideoKontext LDM conditioning -> flow-matching sampler -> AE decode -> remux video。
"""
...
def load_model(checkpoint_path: str):
    """加载 VideoKontext checkpoint 与 SynchformerProcessor。"""
...
def normalize_text(text):
    """对 prompt 使用与 CLAP conditioner 一致的文本后处理。"""
...
def generate(
    ...
    """为输入视频生成匹配音效并重新封装为 mp4。

    Returns:
        list[str]: 临时 mp4 文件路径列表，每个文件包含原视频和生成音频。
    """
    if ldm is None:
...
def main():
    """命令行入口：加载 VFlow checkpoint 并启动 Gradio。"""
