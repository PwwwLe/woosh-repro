"""Woosh-Flow text-to-audio Gradio demo。

加载官方 Woosh-Flow LDM checkpoint，使用 ODE flow-matching sampler 从
``[B, 128, 501]`` latent noise 生成 latent，再通过 Woosh-AE 解码成
48 kHz waveform。
"""
...
def load_model(checkpoint_path: str):
    """加载 Woosh-Flow checkpoint，并移动到 CUDA/MPS/CPU。"""
    global ldm, device
...
def generate(
    ...
    Returns:
        list[tuple[int, np.ndarray]]: Gradio Audio 组件使用的 ``(sr, wav)``。
    """
    if ldm is None:
...
def main():
    """解析 CLI 参数，加载模型并启动 Gradio server。"""
