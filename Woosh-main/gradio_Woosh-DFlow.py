"""Woosh-DFlow text-to-audio Gradio demo。

该 demo 使用 ``FlowMapFromPretrained`` 加载 distilled student，并用少步
Euler/renoise sampler 生成 latent，最后经 Woosh-AE 解码为 48 kHz 音频。
"""
...
def load_model(checkpoint_path: str):
    """加载 Woosh-DFlow checkpoint，并选择可用推理设备。"""
    global ldm, device
...
def generate(
    ...
    """根据 prompt 运行 distilled 采样并返回 Gradio audio 列表。"""
    if ldm is None:
...
def build_ui():
    """构建 Woosh-DFlow 的文本输入、采样参数和音频输出 UI。"""
...
def main():
    """命令行入口：加载 checkpoint 并启动 Gradio。"""
