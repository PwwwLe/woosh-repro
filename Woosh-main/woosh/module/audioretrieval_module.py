"""Woosh-CLAP 的 audio/text retrieval model。

该模块组合 PaSST audio frontend、RoBERTa text frontend 和投影 head，输出
归一化 shared-space embedding。推理脚本会分别加载 text/audio 权重，并可把
文本分支导出为 diffusion conditioner。
"""
...
def get_audio_head_model(
    ...
    """构建 audio embedding projection head。"""
...
def get_sentence_frontend_model(sentence_config):
    """加载 Hugging Face text encoder 与 tokenizer。"""
...
def get_sentence_head_model(
    ...
    """构建 text embedding projection head。"""
...
class AudioRetrievalModel(nn.Module):
    """CLAP-style audio/text 双塔检索模型。

    ``forward`` 接受 ``{"audio": ..., "text": ...}``，返回 ``audio`` 和/或
    ``text`` embedding。音频输入可为路径、waveform Tensor 或 numpy array。
    """
    ...
    def forward_audio_model(self, batch):
        """把批量 waveform 分段送入 audio frontend，写入 ``audio_features``。"""
        # embed audios
    ...
    def _get_audio_embedding(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        """从已整理的 batch 计算归一化 audio embedding。"""
    ...
    def get_audio_embedding_from_data(
        ...
        """从内存中的 waveform array/Tensor 提取 audio embedding。"""
